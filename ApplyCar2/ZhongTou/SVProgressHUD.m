...
//#import "SweepAnimation.h"
#import <QuartzCore/QuartzCore.h>

#define HUDWIDTH    47
#define gifSize 630/2*SCREEN_RATIO_IPHONE6
...
@property (nonatomic, strong, readonly) UIImageView *xiuImageView;
...
@synthesize overlayWindow, hudView, maskType, fadeOutTimer, stringLabel, imageView, spinnerView, visibleKeyboardHeight,animationView,xiuImageView;
...
- (void)showWithMaskType:(SVProgressHUDMaskType)type{
    [self showWithStatus:nil maskType:type networkIndicator:NO];
...
- (void)setStatus:(NSString *)string {
    ...
    if(string) {
        CGSize stringSize = sizeWithText(string, @{NSFontAttributeName: self.stringLabel.font}, CGSizeMake(200, 300));
...
//        CGSize stringSize = sizeWithText(string, @{NSFontAttributeName: self.xiuLabel.font}, CGSizeMake(200, 300));
//        stringWidth = stringSize.width;
//        stringHeight = stringSize.height;
//        hudHeight = 80+stringHeight;
//        
...
//            labelRect = CGRectMake(0, 66, hudWidth, stringHeight);
//        }
        self.xiuImageView.hidden = NO;
//        self.xiuImageView.frame = CGRectMake(11, 22, 48 , 26);
        self.xiuImageView.frame = CGRectMake((HUDWIDTH - 45)/2, (HUDWIDTH - 15)/2, 45,15);
    }
...
//        [self.sweepAnimation startAnimation];
...
- (UIView *)hudView {
    if(!hudView) {
        ...
        hudView.layer.cornerRadius = HUDWIDTH/2;
		hudView.backgroundColor = [UIColor colorWithWhite:1 alpha:0.6];
...
- (UIImageView *)xiuImageView
{
    if (xiuImageView == nil) {
        xiuImageView = [[UIImageView alloc] initWithFrame:CGRectZero];
        xiuImageView.backgroundColor = [UIColor clearColor];
        xiuImageView.frame = CGRectMake(0, 0, gifSize,gifSize);
        
        NSMutableArray *animaImages = [NSMutableArray array];
        for (NSUInteger i = 0; i<=15; i++) {
            UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"Preloader_4_0000%zd", i]];
            [animaImages addObject:image];
        }
        
        xiuImageView.animationImages = animaImages; //动画图片数组
        xiuImageView.animationDuration = animaImages.count * 0.04;// 0.04*15; //执行一次完整动画所需的时长
        xiuImageView.animationRepeatCount = 0;  //动画重复次数
        [xiuImageView startAnimating];
    }
    
    if(!xiuImageView.superview)
        [self.hudView addSubview:xiuImageView];
...
- (void)setIsOnAnimating:(BOOL)isOnAnimating {
    _isOnAnimating = isOnAnimating;
